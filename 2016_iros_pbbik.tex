...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ABSTRACT %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
The paper provides an extensive evaluation of the performance of local descriptors for tracking applications.
A multitude of different descriptors has been proposed in the literature for a wide range of specific applications in Computer Vision such as object recognition and 3D reconstruction. Several works have demonstrated
recently that it is now feasible to employ more complex feature descriptors for online tracking due to fast key-point detectors. While much effort has been spent on evaluating their performance in terms of distinctiveness and robustness to image transformations, very little has been done in the contest of tracking. Our evaluation is performed in terms of distinctiveness, tracking precision and tracking speed. The results show that binary descriptors like ORB or BRISK have comparable results to SIFT or AKAZE. 
...
\textbf{Evaluation.}  There are three criteria the local descriptors are tested upon: distinctiveness, tracking precision and speed. First we measure the distinctiveness by matching the feature descriptors extracted in the first frame of the sequence and by computing the ratio of true positive and false positives. Then we calculate the tracking by integrating each feature descriptor in a key-point based tracker and we calculate the well known overlap accuracy for low, medium and high precision requirements. Last we profile the performance of each descriptor.
